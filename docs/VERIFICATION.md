...
| Automatic Learning | **PENDING managed acceptance.** Intended checkpoints: live API configured with `CPK_INTELLIGENCE_API_KEY` and `CPK_INTELLIGENCE_LEARNING_CONTAINER_ID`, new `default` agent Threads collected in the focused Learning container, manual analysis, supporting Thread review, Skill approval/publication, delivery enabled, and a fresh built-in model invocation showing `copilotkit_load_skill` with the structured result applied. See [Automatic Learning operator guide](LEARNING.md). | No managed Learning recording is claimed here. Sample mode remains key-free. Background task-worker evidence remains outside this increment. External AG-UI `default` Threads may be collected, but external AG-UI skill delivery is owned by that agent server. |

...
## Automatic Learning managed acceptance

September 22, 2026: a live, model-backed OpenMuse API connected to CopilotKit Intelligence with the server-only project key and focused `openmuse-assistant` Learning container. Sixteen completed conversations using public CopilotKit documentation pages were collected as evidence. A manual analysis finished with 2 Insights and 1 Skill candidate from those 16 Threads. The first Insight cites 15 messages across 5 Threads; its supporting messages were reviewed in the Learning UI. The candidate `public-url-research-response` was approved and published as revision 1, and Skill delivery was enabled.

After publication, a fresh OpenMuse side chat researched the public [Automatic Learning page](https://docs.copilotkit.ai/learning). The managed Thread event trace for `ef677ca1-345c-4852-a9ef-61f88728d5df` shows `copilotkit_load_skill` returning the published Skill's `SKILL.md`, followed by `browse_web` reading that page. The completed answer used the Skill's exact five labeled parts: Short answer, Source URL, Key facts, Caveats, and Suggested next step. This Skill has no supporting files, so `copilotkit_read_skill_file` was not expected in this invocation. The fresh Thread became the container's seventeenth collected Thread; it was not part of the 16-Thread analysis.

[Watch the 75-second, 1920 × 1080 acceptance recording](../assets/demos/2026-09-22/learning.mp4). It shows candidate review/publication, the finished analysis and source evidence, published revision and enabled delivery, then the fresh public-source chat and its structured answer. The `copilotkit_load_skill` event was checked separately through the managed Thread inspection API because OpenMuse's chat UI does not render that internal tool call. The recording was visually inspected for keys and unrelated private content; the rejected initial screen capture was permanently discarded.
...
| Automatic Learning | Managed live acceptance and [recording](../assets/demos/2026-09-22/learning.mp4): 16 public-source Threads analyzed; 2 Insights, 1 approved and published Skill, delivery enabled; a fresh built-in model invocation loaded the Skill through `copilotkit_load_skill`, fetched the public page, and followed the five-part format. See [Automatic Learning operator guide](LEARNING.md). | Sample mode remains key-free. Background task-worker evidence remains outside this increment. External AG-UI `default` Threads may be collected, but external AG-UI skill delivery is owned by that agent server. |
